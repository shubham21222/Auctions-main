...
export default function ItemForm6({ setCurrentStep }) {
    const steps = ["Category", "Information", "Photos", "Logistics", "Review"];
    const currentStep = 2;
    const [date, setDate] = useState(null);
    const [isFramed, setIsFramed] = useState(false);
    const [containsRestrictedMaterials, setContainsRestrictedMaterials] = useState(false);
            ...
            <div className="max-w-3xl mx-auto p-6 space-y-6">
                ...
                <Card>
                    <CardContent className="space-y-4 p-6">
                        ...
                        <Input placeholder="Enter artist or maker" />
                        <Label>What is the object?</Label>
                        <Textarea placeholder="Describe the object (max 100 characters)" maxLength={100} />
                        <Label>Medium / Material</Label>
                        <Input placeholder="Enter medium or material" />
                        <div className="flex items-center space-x-2">
                            <Checkbox
                                id="restricted-materials"
                                checked={containsRestrictedMaterials}
                                onCheckedChange={(checked) => setContainsRestrictedMaterials(checked)}
                            />
                            <Label htmlFor="restricted-materials">Piece contains restricted materials such as ivory or tortoiseshell</Label>
                        </div>
                        <Label className="mr-4">Date / Period of Work</Label>
                        <DatePicker
                            selected={date}
                            onChange={(date) => setDate(date)}
                            className="date px-2 rounded-md p-2 w-full"
                        />
                        ...
                        <div className="flex items-center space-x-2">
                            <Checkbox
                                id="is-framed"
                                checked={isFramed}
                                onCheckedChange={(checked) => setIsFramed(checked)}
                            />
                            <Label htmlFor="is-framed">Yes, it is framed</Label>
                        </div>
                        ...
                        <Textarea placeholder="Describe any signatures, labels, or markings" />
