...
import { setToken, setEmail, setUser, setUserId, updateBillingDetails, setEmailVerified } from "@/redux/authSlice";
...
// Step 3: Verify token and get additional user data (including billing details and email verification status)
try {
...
if (verifyStatus && verifyItems && typeof verifyItems === "object") {
  ...
  const billingDetails =
    verifiedUserData.BillingDetails && verifiedUserData.BillingDetails.length > 0
      ? verifiedUserData.BillingDetails[0]
      : {};
  dispatch(updateBillingDetails(billingDetails));
  console.log("Billing details dispatched:", billingDetails);

  // Dispatch email verification status
  const isEmailVerified = verifiedUserData.isEmailVerified || false;
  dispatch(setEmailVerified(isEmailVerified));
  console.log("Email verification status dispatched:", isEmailVerified);
