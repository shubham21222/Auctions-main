  ...
  io.on("connection", (socket) => {
    ...
    socket.on("setAuctionMode", async ({ auctionId, mode, userId }) => {
      try {
        ...
        if (!auctionModes[auctionId] || auctionModes[auctionId] !== mode) {
          auctionModes[auctionId] = mode;
          console.log(`Auction ${auctionId} mode set to: ${mode}`);
          io.to(auctionId).emit("auctionModeUpdate", { auctionId, mode });
        }
    ...
    socket.on("placeBid", async ({ auctionId, userId: bidderId, bidType }) => {
      try {
        ...
        if ((bidType === "competitor" && user.role !== "ADMIN") || 
            (bidType === "online" && user.role === "ADMIN")) {
          return socket.emit("error", { message: "Invalid bid type for user role." });
        }

        const currentMode = auctionModes[auctionId] || "online";
        if ((bidType === "competitor" && currentMode !== "competitor") || 
            (bidType === "online" && currentMode !== "online")) {
          return socket.emit("error", { message: "Auction is not in the correct bid mode." });
        }

        let ipAddress = socket.handshake.headers["x-forwarded-for"] || socket.handshake.address;
        ipAddress = ipAddress.includes(",") ? ipAddress.split(",")[0].trim() : ipAddress;
        if (ipAddress.startsWith("::ffff:")) ipAddress = ipAddress.split(":").pop();

        const getBidIncrement = (currentBid) => {
          return currentBid >= 1000000 ? 50000 :
                 currentBid >= 500000 ? 25000 :
                 currentBid >= 250000 ? 10000 :
                 currentBid >= 100000 ? 5000 :
                 currentBid >= 50000 ? 2500 :
                 currentBid >= 25000 ? 1000 :
                 currentBid >= 10000 ? 500 :
                 currentBid >= 5000 ? 250 :
                 currentBid >= 1000 ? 100 :
                 currentBid >= 100 ? 50 :
                 currentBid >= 50 ? 10 :
        ...
        const currentBid = auction.currentBid || 0;
        const bidIncrement = getBidIncrement(currentBid);
        const bidRule = await bidIncrementModel.findOne({ price: { $lte: currentBid + bidIncrement } }).sort({ price: -1 });
        const requiredIncrement = bidRule ? bidRule.increment : bidIncrement;
        const finalBidAmount = currentBid + requiredIncrement;
        ...
        console.log(`Bid placed: ${finalBidAmount} by ${bidderId} on auction ${auctionId} (Type: ${bidType || "online"})`);
        io.in(auctionId).emit("bidUpdate", {
          auctionId,
          bidAmount: finalBidAmount,
          bidderId,
        ...
        if (lastBidderId && lastBidderId.toString() !== bidderId) {
          userSocketMap[lastBidderId]?.forEach((socketId) => {
            io.to(socketId).emit("outbidNotification", {
              message: "You've been outbid!",
              auctionId,
            });
          });
